...
Step PlannerClass::ChangeToStep(float* offset, uint8_t index)
{
	...
	offset[index] -= LastError[index];

	if (offset[index] >= 0)
	{
		stepBuffer.Direction = INCREASE;
		offsetStep = offset[index];
	}
	else
	{
	...
	stepBuffer.StepsToJump = roundf(offsetStep * StepsPerDeg[index]);

	LastError[index] = ((float)stepBuffer.StepsToJump / StepsPerDeg[index]) - offsetStep;
